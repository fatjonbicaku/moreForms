    ...
    <>
      <form className='container' >
        <div>
          ...
          <input type="text" placeholder='Search' name="" id="" onChange={HandleName} />
        </div>
        <p>{countFn > 0 && countFn <= 2 ? "First Name must be at least 2 characters" : ""}</p>
        <div>
          <label htmlFor="">Last Name</label>
          <input type="text" placeholder='Search' name="" id="" onChange={HandleLname} />
        </div>
        <p>{countLn > 0 && countLn <= 2 ? "Last Name must be at least 2 characters" : ""}</p>
        <div>
          <label htmlFor="">Email:</label>
          <input type="email" placeholder='Search' name="" id="" onChange={HandleEm} />
        </div>
        <p>{countEm > 0 && countEm <= 5 ? "Email must be at least 5 characters" : ""}</p>
        <div>
          <label htmlFor="">Password:</label>
          <input type="password" placeholder='Search' name="" id="" onChange={HandlePass} />
        </div>
        <p>{countPass.length > 0 && countPass.length <= 6 ? "Password must be at least 6 characters" : ""}</p>
        
        <div>
          <label htmlFor="">Confirm Password:</label>
          <input type="password" placeholder='Search' name="" id="" onChange={HandlePassCon} />
        </div>
